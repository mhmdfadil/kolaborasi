...
<div class="modal fade" id="addDokumentasiModal" tabindex="-1" aria-labelledby="addDokumentasiModalLabel" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <!-- Form Modal -->
            <form action="{{ route('store-dokumentasi') }}" method="POST" enctype="multipart/form-data">
                @csrf
...
<div class="modal fade" id="editModal{{ $partnership->id }}" tabindex="-1" aria-labelledby="editModalLabel{{ $partnership->id }}" aria-hidden="true">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <!-- Form Modal -->
            <form action="{{ route('update-dokumentasi', $partnership->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <div class="modal-body">
                    <!-- Input partnership_id (diset ke ID kerjasama yang ada) -->
                    <div class="mb-3">
                        <label for="partnership_id{{ $partnership->id }}" class="form-label">Partnership ID</label>
                        <select class="form-select" id="partnership_id{{ $partnership->id }}" name="partnership_id" required>
                            <option value="" disabled>Pilih Kerjasama</option>
                            @foreach($amitra as $organisation)
                                <option value="{{ $organisation->id }}" {{ $partnership->partnership_id == $organisation->id ? 'selected' : '' }}>
                                    {{ $organisation->partnership_type }}
                                </option>
                            @endforeach
                        </select>
                    </div>
            ...
            @foreach($adokumentasi as $key => $partnership)
                <tr>
                    ...
                    <td class="text-center"  width="100px">
                        ...
                        <form action="{{ route('delete-dokumentasi', $partnership->id) }}" method="POST" style="display:inline;" id="delete-form-{{ $partnership->id }}">
                            @csrf
                            @method('DELETE')
                            <button class="btn btn-danger btn-sm" type="button" onclick="confirmDelete({{ $partnership->id }})">
...
<script>
    ...
    function confirmDelete(partnershipId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                document.getElementById('delete-form-' + partnershipId).submit();
            }
        });
    }
</script>
